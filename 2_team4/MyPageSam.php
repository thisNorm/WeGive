...
// 역할에 따라 쿼리를 결정
if ($role === 'users') {
    $sql = "SELECT USER_ID, NAME, EMAIL, TO_CHAR(BIRTH_DAY, 'YYYY-MM-DD') AS BIRTH_DAY FROM USERS WHERE USER_ID = :id";
} elseif ($role === 'giver') {
    $sql = "SELECT 
...
<div class="mypage-container1">
    <h2 class="mypage-h2">마이페이지 - <?php echo $role === 'giver' ? '나눔러' : '사용자'; ?></h2>
                            ...
                            <td>
                                ...
                                <?php elseif ($transaction['STATUS'] === 'IN_PROGRESS'): ?>
                                    ...
                                    <?php elseif ($transaction['STATUS'] === 'COMPLETED' && $role === 'users'): ?>
                                        <?php if ($transaction['IS_REVIEWED']): ?>
                                            <!-- 리뷰가 이미 작성된 경우 -->
                                            <button class="mypage-review-complete-btn" disabled>작성 완료</button>
                                        <?php else: ?>
                                            <!-- 리뷰가 작성되지 않은 경우 -->
...
xhr.onload = function () {
    if (xhr.status === 200) {
        ...
        const reviewButton = document.querySelector(`[data-transaction_id="${transactionId}"]`);
        if (reviewButton) {
            reviewButton.textContent = "작성 완료";
            reviewButton.disabled = true;  // 버튼 비활성화
            reviewButton.classList.add("review-complete-btn");  // 스타일 적용
        }
    } else {
        // 네트워크 오류 시
