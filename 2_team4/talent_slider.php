...
LEFT JOIN 
    (SELECT 
        t.giver_id, 
        COUNT(CASE WHEN t.status = 'COMPLETED' THEN 1 END) AS TRADE_VOLUME
     FROM 
        transaction t
     GROUP BY t.giver_id) t
    ON p.GIVER_ID = t.giver_id
ORDER BY 
    COALESCE(t.TRADE_VOLUME, 0) DESC, 
    COALESCE(r.REVIEW_SCORE, 0) DESC,
    p.TITLE ASC
...
/* 새로운 컨테이너 스타일 */
.talent-container {
    background-color: #f2f3f7; /* 새로운 배경색 */
    padding: 10px;
    margin-top: 50px; /* 위쪽 여백 */
}


/* 기존 talent-slider-container 스타일 유지 */
...
.talent-item img {
    ...
    height: 180px;
    object-fit: cover;
    border-radius: 20px;  /* 이미지를 둥글게 만들기 */
    border: 3px solid #878282; /* 테두리 추가, 두께 3px, 색상 회색 */
...
/* 버튼 스타일 */
.slider-button {
    background-color: rgba(0, 0, 0, 0); /* 버튼 배경색 */
    color: black;
    border: none;
